...
services:
    ...
    spring-boot:
        image: dinhsi1401/pharmacy:2.0         # Tải trực tiếp image Backend đã được build sẵn
        ...
        environment:                           # Cấu hình biến môi trường kết nối Database
            ...
            SPRING_MONGODB_URI: mongodb://root:root@mongodb:27017/pharmacy?authSource=admin
            SPRING_DATA_MONGODB_URI: mongodb://root:root@mongodb:27017/pharmacy?authSource=admin
...
Chạy lệnh sau để Docker tự động tải các image, build mã nguồn frontend và khởi động ngầm (`-d`):
```bash
sudo docker compose up -d --build
```
> Lệnh này sẽ thực hiện build multi-stage cho Frontend dựa trên cấu hình [Dockerfile](file:///d:/pharmacy-project_/frontend/Dockerfile), nén code React thành file tĩnh và nhúng vào image Web Server Nginx.
